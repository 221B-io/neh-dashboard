<template>
  <div class="animated fadeIn">
    ...
    <b-card-group columns class="card-columns cols-2">
      ...
      <b-card header="Missing Products">
        <div class="chart-wrapper">
          <pie-resolved/>
        </div>
      </b-card>
...
export default {
  ...
  created() {
    axios.get('http://localhost:8080/api/institutions').then(results => {
      ...
      // console.log(this.institutionNames);
      // debugger;
      this.institutionNumbers = results.data.map(x => x.doc_count);
      console.log(this.instutionNumbers);
    ...
    // query the most recent grants
    const body = bodybuilder().build();
    // axios.post('http://localhost:3000/api/raw', body).then();
  },
  methods: {
    variant(value) {
      let $variant;
